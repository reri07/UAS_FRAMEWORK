<template>
  <div>
    <div class="container mx-auto">
      <p class="font-semibold text-center mt-2 mb-5">Matakuliah</p>
      <NuxtLink
        to="/matakuliah/addmatakuliah"
        ...
        <table class="w-full text-sm text-left text-slate-500">
          <thead
            ...
            <tr>
              ...
              <th scope="col" class="px-6 py-4">Jumlah SKS</th>
              <th scope="col" class="px-6 py-4">Aksi</th>
            </tr>
          </thead>
          <tbody id="example">
            <tr
              v-for="(row, index) in data_matakuliah"
              :key="row.id"
              class="bg-white border-b hover:bg-slate-100 text-center">
              <td class="px-6 py-4">{{ index + 1 }}</td>
              <td class="px-6 py-4">{{ row.id }}</td>
              <td class="px-6 py-4">
                <a :href="'/matakuliah/' + row.id">{{ row.matakuliah }}</a>
              </td>
              <td class="px-6 py-4">{{ row.sks }}</td>
              <td class="px-3 py-2 mx-auto">
                <div class="grid lg:grid-cols-2 md:grid-cols-1 w-28 mx-auto">
                  <NuxtLink
                    :to="'/matakuliah/update/' + row.id"
                  ...
                  <button
                    type="button"
                    @click="($event) => deleteMatakuliah($event, row.id)"
...
<script>
  ...
  export default {
    ...
    methods: {
      getDataMatakuliah() {
        axios.get(BaseUrl + "/matakuliah").then((res) => {
          this.data_matakuliah = res.data;
        });
      },
      deleteMatakuliah(event, matakuliahId) {
        if (confirm("Apakah anda yakin ingin menghapus data ini ?")) {
          event.target.innerText = "Deleting";
          axios.delete(BaseUrl + `matakuliah/${matakuliahId}`).then((res) => {
